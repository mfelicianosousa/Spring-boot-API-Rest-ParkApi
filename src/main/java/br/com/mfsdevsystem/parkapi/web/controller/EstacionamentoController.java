...
import br.com.mfsdevsystem.parkapi.service.ClienteVagaService;
import br.com.mfsdevsystem.parkapi.service.EstacionamentoService;
import br.com.mfsdevsystem.parkapi.service.JasperService;
...
import br.com.mfsdevsystem.parkapi.web.dto.mapper.ClienteVagaMapper;
...
@Tag( name = "Estacionamentos", description = "Operações de registro de entrada e saída de um veiculo do estacionamento.")
@RestController
...
public class EstacionamentoController {
	...
	@Operation( summary="Operação de check-in", description="Recurso para dar entrada de um veiculo no estacionamento."+
			...
			responses = {
					...
					@ApiResponse( responseCode = "422", description = "Recurso não processado por dados de entrada inválidos",
					    	content = @Content(mediaType="application/json; charset=UTF-8",
					    	schema = @Schema( implementation = ErrorMessage.class ))),
					@ApiResponse( responseCode = "403", description = "Recurso não pemitido ao perfil de ADMIN",
        					content = @Content(mediaType="application/json; charset=UTF-8",
        					schema = @Schema( implementation = ErrorMessage.class ))),
			}		
	)
	...
	@Operation( summary="Localizar um veiculo estacionado", description="Recurso para retornar um veiculo estacionado."+
			"pelo número do recibo. Requisição exige uso de um bearer token.",
			security = @SecurityRequirement(name="security"),
			parameters = {
	...
	public ResponseEntity<EstacionamentoResponseDto> getByRecibo(@PathVariable String recibo){
	 ...
	 @Operation( summary="Operação de check-out", description="Recurso para dar saída de um veiculo do estacionamento."+
			"Requisição exige uso de um bearer token. Acesso restrito a ROLE = 'ADMIN'",
			security = @SecurityRequirement(name="security"),
			parameters = {
					@Parameter( in = PATH, name ="recibo", description="Número do recibo gerado pelo check-in")
			...
			responses = {			
					@ApiResponse( responseCode = "200", description ="Recurso atualizado com sucesso.",
							content = @Content(mediaType="application/json; charset=UTF-8",
							schema = @Schema(implementation = EstacionamentoResponseDto.class))),
					@ApiResponse( responseCode = "404", description = "Número do recibo inexistente ou o "+
							"veiculo já passou pele checkout",
					    	content = @Content(mediaType="application/json; charset=UTF-8",
					    	schema = @Schema( implementation = ErrorMessage.class ))),
